    ...
    <div class="sidebar-wrapper">
        <nav class="mt-2"> <!--begin::Sidebar Menu-->
            <ul class="nav sidebar-menu flex-column" data-lte-toggle="treeview" role="menu" data-accordion="false">
                @if(auth()->user()->hasPermission('auth'))
                        ...
                        <ul class="nav nav-treeview">
                            <li class="nav-item {{ isActive('/admin-panel/products') }}">
                                <a href="/admin-panel/products"
                                   class="nav-link {{ isUrl('/admin-panel/products')}}">
                ...
                @endif
                @if(auth()->user()->hasPermission('database-backup') || auth()->user()->hasPermission('database-recovery'))
                    <li class="nav-item {{ isActive(['/admin-panel/database/backup', '/admin-panel/database/recovery'], "menu-open") }}">
                        <a href="#" class="nav-link"> <i
                                    class="nav-icon bi bi-database-fill-gear"></i>
                            <p>
                                Database
                                <i class="nav-arrow bi bi-chevron-right"></i>
                            </p>
