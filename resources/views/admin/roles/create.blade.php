@component('admin.layouts.content', ['title' => 'Create New Role'])
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <form action="/admin-panel/roles/store" method="POST">
                    <div class="card-body">
                        <div class="form-group pb-3">
                            <label for="name" class="col-sm-2 control-label">Roles Name</label>
                            <input type="text" name="name" class="form-control" id="name"
                                   placeholder="Enter role's name" autocomplete="name" value="{{ $old($name) }}"
                                   required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group pb-3">
                            ...
                            <input type="text" name="label" class="form-control" id="label"
                                   placeholder="Enter role's label" autocomplete="name" value="{{ $old($label) }}"
                                   required>
                            @if($errors->has('label'))
